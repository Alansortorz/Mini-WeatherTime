...
//程序连接WiFi密码
//const char *wifi_ssid = "484-1-2.4G";
//const char *wifi_passward = "l484484484";
const char *bilibili_id = "491344524";
String fans = "-1"; 
...
const char *qweather_key = "d4ee3cc90a9a4fbba9f4fc5422c940a3";//和风天气秘钥
//城市ID查询网站：https://where.qweather.com/
const char *locaton = "101180109";//城市ID 101230207 厦门市翔安区
//如何根据IP地址自动定位地址信息
//const char *locaton = "auto_ip";
const int UPDATE_INTERVAL_SECS = 240 * 60; // 240分钟更新一次天气
...
//OLED
const int SDA_PIN = 3;  //引脚连接
const int SDC_PIN = 1;  //
SSD1306Wire display(0x3c, SDA_PIN, SDC_PIN);
...
void webconnect();
void app_SmartConfig();
bool AutoConfig();
//添加框架
//此数组保留指向所有帧的函数指针
//框架是从右向左滑动的单个视图
FrameCallback frames[] = { drawDateTime, drawCurrentWeather };
//页面数量
...
void setup() {
  ...
  //设置屏幕方向：SLIDE_LEFT, SLIDE_RIGHT, SLIDE_TOP, SLIDE_DOWN
...
void loop() {
  ...
  if(millis() - timeSinceLastCurrUpdate > (1000L * UPDATE_CURR_INTERVAL_SECS)) { //粉丝数更新
    HeFengClient.fans(&currentWeather, bilibili_id);
...
void webconnect() {  ////Web配网，密码直连将其注释
  ...
  display.display();
  //display.drawString(0, 0, WiFi.localIP());
  ...
  if (!wifiManager.autoConnect("PangHong-IOT-Display")) {  //AP模式
...
void updateData(OLEDDisplay *display) {  //天气更新
  HeFengClient.doUpdateCurr(&currentWeather, qweather_key, locaton);
  HeFengClient.doUpdateFore(foreWeather, qweather_key, locaton);
...
void updateDatas(OLEDDisplay *display) {  //首次天气更新
  ...
  //HeFengClient.doUpdateFore(foreWeather, qweather_key, locaton);
  readyForWeatherUpdate = false;
  drawProgress(display, 100, "loading...");
  delay(200);
}
...
void drawHeaderOverlay(OLEDDisplay *display, OLEDDisplayUiState* state) {   //绘图页眉覆盖
  ...
  //display->drawString(100, 54, "cocer:");
  String temp = fans;
  display->drawString(112, 54, temp);
